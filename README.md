...
* The DNANexus file ID of a saved FusionInspector Docker image, which should be a compressed '.tar.gz'
...
* Loads and runs the FusionInspector Docker image for each known fusions file as a child job in DNANexus:
...
* Merged the output file from each child job output.
* Creates the html report.
* Uploads the output files to DNANexus.
...
* The following outputs are produced both with and without Trinity being run:
    ...
    * fi_inspected_fusions: a text file containing fusions which were inspected by FusionInspector
    * fi_missed_fusions: a text file containing fusions which were not inspected by FusionInspector. This is a limitation of FusionInspector 
    documented on their github repo (https://github.com/FusionInspector/FusionInspector/issues/83)
